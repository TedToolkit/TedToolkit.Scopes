...
internal class ScopeTests
{
    ...
    {
        var baseCount = count * 10;
        using (new Scope<ValueScope>(new ValueScope(baseCount + 1)))
        {
            await Assert.That( Scope<ValueScope>.Current?.Value).IsEqualTo(baseCount + 1);
        ...
        Task CreateScopeTest(int value)
        {
            return Task.Run(async () =>
            {
                using (new Scope<ValueScope>(new ValueScope(value)))
                {
                    await Assert.That(Scope<ValueScope>.Current?.Value).IsEqualTo(value);
                    await Task.Delay(100);
                    await Assert.That(Scope<ValueScope>.Current?.Value).IsEqualTo(value);
                }
            });
